...
- 4.2 Build vs select Analysis Framework
...
```
START: Need to select technology for Layer Y

├─ Healthcare deployment?
│  ├─ YES: Filter to HIPAA-eligible only (see Part 3.1)
│  │  └─ Budget tier?
│  │     ├─ Tier 1 ($30-50K): Use lean budget options
│  │     ├─ Tier 2 ($150K): Use moderate budget options (recommended)
│  │     └─ Tier 3 ($300K+): Use well-funded options
│  └─ NO: General enterprise
│     └─ Cloud platform committed?
│        ├─ AWS: Prefer AWS-native options
│        ├─ Azure: Prefer Azure-native options
│        ├─ GCP: Prefer GCP-native options
│        └─ Multi-cloud: Prefer cloud-agnostic options

├─ Evaluate INPACT™ + GOALS scores:
│  ├─ Healthcare: Require INPACT™ ≥28, GOALS ≥20
│  ├─ Enterprise: Require INPACT™ ≥24, GOALS ≥16
│  └─ Internal tools: INPACT™ ≥18, GOALS ≥11 acceptable

├─ Check prerequisites:
│  ├─ Team has expertise? (A score matters)
│  ├─ Integrations exist? (C score matters)
│  └─ Budget approved? (Cost vs value)

└─ DECISION: Technology selected
   └─ Document in Pre-Flight Readiness (Chapter 3)
```
...
**Use this to decide: "Should we build this ourselves or select a product?"**
